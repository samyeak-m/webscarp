...
public class LiveData {
        ...
        while (true) {
            try {
                ...
                if (!currentHash.equals(lastHash)) {
                    ...
                    // Store end-of-day data if it's closing time
                    if (now.getHour() >= CLOSE_HOUR) {
                        storeDailyData(content);
                    }
                } else {
    ...
    private static void createTablesIfNotExists() throws SQLException {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
            ...
            String sqlLiveData = "CREATE TABLE IF NOT EXISTS live_data (" +
                    ...
                    ")";
            stmt.executeUpdate(sqlLiveData);

            String sqlDailyData = "CREATE TABLE IF NOT EXISTS daily_data (" +
                    "id INT AUTO_INCREMENT PRIMARY KEY," +
                    "date DATE," +
                    "symbol VARCHAR(255)," +
                    "open DECIMAL(10,2)," +
                    ...
                    "UNIQUE KEY unique_date_symbol (date, symbol)" +
                    ")";
            stmt.executeUpdate(sqlDailyData);
        }
    }
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS)) {
            ...
            for (Element row : doc.select("table.table tr")) {
                ...
                if (cells.size() >= 7) {
                    ...
                    String sql = "INSERT INTO live_data (date, symbol, open, high, low, close, volume) " +
                            "VALUES (?, ?, ?, ?, ?, ?, ?) " +
    ...
    private static void storeDailyData(String content) throws SQLException {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS)) {
            Document doc = Jsoup.parse(content);
            for (Element row : doc.select("table.table tr")) {
                Elements cells = row.select("td");
                if (cells.size() >= 7) {
                    ...
                    String sql = "INSERT INTO daily_data (date, symbol, open, high, low, close, volume) " +
                            "VALUES (?, ?, ?, ?, ?, ?, ?) " +
                            "ON DUPLICATE KEY UPDATE open = VALUES(open), high = VALUES(high), low = VALUES(low), close = VALUES(close), volume = VALUES(volume)";
